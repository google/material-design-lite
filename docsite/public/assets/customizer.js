...
MaterialCustomizer = (function () {
  ...
  MaterialCustomizer.prototype.generateFieldTemplate_ = function () {
    ...
    darkField.setAttribute(
      ...
      [
        ...
        [
          (config.ri + config.hd) *
            Math.sin(config.alphaRad + config.alphaIncr),
          -(config.ri + config.hd) *
            Math.cos(config.alphaRad + config.alphaIncr),
        ].join(","),
        [0, -(config.ri + config.hd)].join(","),
        [0, -config.ri].join(","),
      ].join(" ")
  ...
  MaterialCustomizer.prototype.fieldClicked_ = function (ev) {
    ...
    if (
      (g.getAttribute("class") || "").indexOf("selected--1") !== -1 &&
      numSelected === 1
    ) {
      return;
    }
  ...
  MaterialCustomizer.prototype.highlightField = function (color) {
    ...
    g.setAttribute(
      "class",
      "selected selected--" + (this.getNumSelected() + 1)
    );
  ...
  MaterialCustomizer.prototype.processTemplate = function (
    primaryColor,
    secondaryColor
  ) {
  ...
  MaterialCustomizer.prototype.calculateContrast = function (
    background,
    foreground
  ) {
