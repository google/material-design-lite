...
(function() {
   ...
   * @private
   */
  MaterialDatePicker.prototype.Keycodes_ = {
    TAB: 9,
    ENTER: 13,
    ESCAPE: 27,
    UP_ARROW: 38,
   ...
   * @return {void}
   */
  MaterialDatePicker.prototype.bindKeyboardEvents_ = function() {
    if (!this.boundDocumentKeydownHandler_) {
      this.boundDocumentKeydownHandler_ = this.documentKeydownHandler_.bind(this);
    }
    document.addEventListener(this.Event_.KEYDOWN, this.boundDocumentKeydownHandler_);
  };
   ...
   * @return {void}
   */
  MaterialDatePicker.prototype.unbindKeyboardEvents_ = function() {
    if (this.boundDocumentKeydownHandler_) {
      document.removeEventListener(this.Event_.KEYDOWN, this.boundDocumentKeydownHandler_);
    }
  };
  ...
  /**
   * Date picker input change handler
   * @param {Event} e change event object
   * @private
   * @return {void}
   */
  MaterialDatePicker.prototype.inputChangeHandler_ = function(e) {
    if (!this.input_.value) {
      this.setSelectedDate(null);
    }
  };
   ...
   * @param  {Event} e Event handler object
   * @private
   * @return {void}
   */
  MaterialDatePicker.prototype.documentKeydownHandler_ = function(e) {
    var keyMatch = true;
    switch (e.keyCode) {
      case this.Keycodes_.DOWN_ARROW:
        this.switchToNextWeek_();
        break;
      case this.Keycodes_.UP_ARROW:
        this.switchToLastWeek_();
        break;
      case this.Keycodes_.LEFT_ARROW:
        this.switchToYesterday_();
        break;
      case this.Keycodes_.RIGHT_ARROW:
        this.switchToTomorrow_();
        break;
      case this.Keycodes_.ENTER:
        if (this.pickedDate_) {
          this.okHandler_(e);
        }
        break;
      case this.Keycodes_.ESCAPE:
        this.close();
        break;
      case this.Keycodes_.TAB:
        // Actually do nothing, just prevent propagation
        break;
      default:
        keyMatch = false;
        break;
  ...
  MaterialDatePicker.prototype.changeCurrentMonth_ = function(nextMonth) {
    ...
    for (var i = 0; i < dateButtons.length; i++) {
      ...
      dateButton.removeEventListener(this.Event_.CLICK, this.boundPickDateHandler);
    }
  ...
  /**
   * Change picked date to last week (same day of the week)
   * @private
   * @return {void}
   */
  MaterialDatePicker.prototype.switchToLastWeek_ = function() {
    this.addDaysToPickedDate_(-7);
  };

  /**
   * Change picked date to next week (same day of the week)
   * @private
   * @return {void}
   */
  MaterialDatePicker.prototype.switchToNextWeek_ = function() {
    this.addDaysToPickedDate_(7);
  };

  /**
   * Change picked date to yesterday
   * @private
   * @return {void}
   */
  MaterialDatePicker.prototype.switchToYesterday_ = function() {
    this.addDaysToPickedDate_(-1);
  };

  /**
   * Change picked date to tomorrow
   * @private
   * @return {void}
   */
  MaterialDatePicker.prototype.switchToTomorrow_ = function() {
    this.addDaysToPickedDate_(1);
  };

  /**
   * Change days to picked date
   * @private
   * @return {void}
   */
  MaterialDatePicker.prototype.addDaysToPickedDate_ = function(addDays) {
    var nextPickedDate = new Date(this.pickedDate_.getTime());
    if (addDays > 0) {
      nextPickedDate.setDate(nextPickedDate.getDate() + addDays);
    } else {
      nextPickedDate.setDate(nextPickedDate.getDate() - Math.abs(addDays));
    }
    if (!this.isInRange_(nextPickedDate)) {
      if (addDays === 1 && this.minDate_) {
        nextPickedDate = new Date(this.minDate_.getTime());
      } else if (addDays === -1 && this.maxDate_) {
        nextPickedDate = new Date(this.maxDate_.getTime());
      }
    }
    if (this.isInRange_(nextPickedDate)) {
      this.pickedDate_ = nextPickedDate;
      this.currentMonth_.setDate(1);
      this.currentMonth_.setMonth(nextPickedDate.getMonth());
      this.currentMonth_.setFullYear(nextPickedDate.getFullYear());
      this.changeCurrentMonth_(this.currentMonth_);
      this.updateHeader_();
    }
  };
  ...
  MaterialDatePicker.prototype.renderHeader_ = function() {
    if (!this.headerElement_) {
      ...
      this.headerYearElement_.addEventListener(this.Event_.CLICK, this.boundHeaderYearClickHandler);
      this.headerDateElement_.addEventListener(this.Event_.CLICK, this.boundHeaderDateClickHandler);
  ...
  MaterialDatePicker.prototype.renderNavigation_ = function() {
    if (!this.navigationElement_) {
      ...
      previousMonth.addEventListener(this.Event_.CLICK, this.boundPreviousMonthHandler, true);
      nextMonth.addEventListener(this.Event_.CLICK, this.boundNextMonthHandler, true);
      ...
      for (var i = 0; i <= 6; i++) {
        ...
        if (currentDay.getDay() === weekStart && currentDay.getDate() <= lastDay.getDate()) {
          ...
          if (!weekDay.classList.contains(this.CssClasses_.DATE_DISABLED)) {
            ...
            weekDay.addEventListener(this.Event_.CLICK, this.boundPickDateHandler, true);
    ...
    if (this.yearPickerElement_) {
      ...
      for (var i = 0; i < yearButtons.length; i++) {
        ...
        if (this.isYearInRange_(existingYear)) {
          ...
          yearButton.removeEventListener(this.Event_.CLICK, this.boundPickYearHandler);
          yearButton.addEventListener(this.Event_.CLICK, this.boundPickYearHandler);
        } else {
          yearButton.removeEventListener(this.Event_.CLICK, this.boundPickYearHandler);
  ...
  MaterialDatePicker.prototype.renderActions_ = function() {
    if (!this.actionsElement_) {
      ...
      this.cancelElement_.addEventListener(this.Event_.CLICK, this.boundCancelActionHandler, true);
      this.okElement_.addEventListener(this.Event_.CLICK, this.boundOkActionHandler, true);
  ...
  MaterialDatePicker.prototype.destroy_ = function() {
    ...
    if (this.cancelElement_) {
      ...
      this.cancelElement_.removeEventListener(this.Event_.CLICK, this.boundCancelActionHandler);
    }
    if (this.okElement_) {
      componentHandler.downgradeElements(this.okElement_);
      this.okElement_.removeEventListener(this.Event_.CLICK, this.boundOkActionHandler);
    }
    if (this.actionsElement_) {
    ...
    if (this.navigationElement_) {
      ...
      if (previousMonth) {
        previousMonth.removeEventListener(this.Event_.CLICK, this.boundPreviousMonthHandler);
      }
      if (nextMonth) {
        nextMonth.removeEventListener(this.Event_.CLICK, this.boundNextMonthHandler);
      }
      this.navigationElement_ = null;
    }
    if (this.headerDateElement_) {
      this.headerDateElement_.removeEventListener(this.Event_.CLICK, this.boundHeaderDateClickHandler);
      this.headerDateElement_ = null;
    }
    if (this.headerYearElement_) {
      this.headerYearElement_.removeEventListener(this.Event_.CLICK, this.boundHeaderYearClickHandler);
   ...
   * @public
   * @return {Date|null} Current selected date
   */
  MaterialDatePicker.prototype.setSelectedDate = function(selectedDate) {
    if (selectedDate) {
      if (this.isInRange_(selectedDate)) {
      ...
      // Reset currently rendered month
      var today = new Date();
      this.currentMonth_.setDate(1);
      this.currentMonth_.setMonth(today.getMonth());
      this.currentMonth_.setFullYear(today.getFullYear());
      // Remove "dirty" class indicator
  ...
  MaterialDatePicker.prototype.init = function() {
    if (this.element_) {
      ...
      if (this.input_) {
        ...
        this.boundInputChangeHandler = this.inputChangeHandler_.bind(this);
        this.input_.addEventListener('focus', this.boundInputFocusHandler, true);
        this.input_.addEventListener('blur', this.boundInputBlurHandler, true);
        this.input_.addEventListener('change', this.boundInputChangeHandler, true);
      }
      ...
      if (this.element_.classList.contains(this.CssClasses_.FIXED)) {
        ...
        this.backdrop_.addEventListener(this.Event_.CLICK, function(e) {
          e.preventDefault();
          e.stopPropagation();
          e.stopImmediatePropagation();
          this.close();
  ...
  MaterialDatePicker.prototype.mdlDowngrade_ = function() {
    if (this.input_) {
      ...
      this.input_.removeEventListener('blur', this.boundInputBlurHandler, true);
      this.input_.removeEventListener('change', this.boundInputChangeHandler, true);
    }
    this.destroy_();
    if (this.backdrop_) {
